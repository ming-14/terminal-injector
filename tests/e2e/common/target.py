...
每个测试文件把目标脚本正文（body）内嵌为字符串，运行时：
  1. write_target(name, body) 生成 _targets/<name>.py（body 前拼接 TARGET_PREAMBLE）
  2. 在注入的 cmd 中执行：python <abs_path> <result_path>
  3. 目标脚本用 rec()/check() 写结果文件，runner 侧轮询断言

TARGET_PREAMBLE 提供共享的 ctypes Console API 绑定，目标正文只需写业务逻辑。
"""
import os
...
# ============================================================
# 共享目标脚本前导：ctypes Console API 全量绑定
# ============================================================
TARGET_PREAMBLE = r'''# -*- coding: utf-8 -*-
"""目标脚本（自动生成，勿手改）。"""
import os
import sys
import time
...
def write_target(name: str, body: str) -> str:
    """生成目标脚本 _targets/<name>.py（前导 + 正文），返回绝对路径。"""
    paths.ensure_dirs()
