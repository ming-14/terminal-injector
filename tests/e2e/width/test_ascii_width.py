"""特性: ASCII 单宽字符光标推进    类别: width

链路: 目标 WriteConsoleW("Hello") → DLL W→VT 翻译 + wcwidth 单宽 →
      DLL 虚拟光标 X 推进 5 → 退出时 ChildExitSync 上报 cursor

预期（wcwidth 集成，必须 PASS）:
  - ChildExitSync sent cursor=(基线.X + 5, <Y>)
    ...
    try:
        with TestSession() as s:
            ...
            if not v:
                ...
                if ok and written == 5:
                    print("  [PASS] WriteConsoleW 写入 5 字符")
                else:
                    print("  [FAIL] WriteConsoleW: ok={} written={}（期望 1/5）".format(
                        ok, written))
                    failures += 1
                ...
                if not cur:
                    ...
                    print("  [INFO] aligned=({},{}) ChildExitSync=({},{}) 期望 X={}".format(
                        bx, by, x, y, exp_x))
                    if x == exp_x:
                        print("  [PASS] \"Hello\" 后 X={}（基线 {}+5 列单宽）".format(
                            x, bx))
                    else:
