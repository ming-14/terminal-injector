"""特性: CJK 双宽字符光标推进    类别: width

链路: 目标 WriteConsoleW("测试") → DLL W→VT 翻译 + wcwidth 双宽 →
      DLL 虚拟光标 X 推进 4 → 退出时 ChildExitSync 上报 cursor

预期（wcwidth 集成，必须 PASS）:
  - ChildExitSync sent cursor=(基线.X + 4, <Y>)（2 字 × 2 列）
    ...
    try:
        with TestSession() as s:
            ...
            if not v:
                ...
                if ok and written == 2:
                    print("  [PASS] WriteConsoleW 写入 2 字符")
                else:
                    print("  [FAIL] WriteConsoleW: ok={} written={}（期望 1/2）".format(
                        ok, written))
                    failures += 1
                ...
                if not cur:
                    ...
                    print("  [INFO] aligned=({},{}) ChildExitSync=({},{}) 期望 X={}".format(
                        bx, by, x, y, exp_x))
                    if x == exp_x:
                        print("  [PASS] \"测试\" 后 X={}（基线 {}+2 字 × 2 列双宽）".format(
                            x, bx))
                    else:
