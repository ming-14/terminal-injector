...
time.sleep(2.0)
h_out = get_std_out()
set_mode(h_out, ENABLE_PROCESSED_OUTPUT)   # WRAP_AT_EOL 关闭
width = get_csbi(h_out).dwSize.X
_k.SetConsoleCursorPosition(h_out, COORD(0, 10))
ok, nw = write_str(h_out, "W" * (width + 2))
info = get_csbi(h_out)
rec("BEFORE_DSR", str(info.dwCursorPosition.X) + " " + str(info.dwCursorPosition.Y))
# 发 DSR 查询 WT 真实光标
okb, nb = write_bytes(h_out, b"\x1b[6n")
rec("DSR_SENT", str(int(okb)))
done()
