"""特性: Kitty 键盘协议查询（CSI > 1 u）    类别: special_sequences

链路: 目标 WriteFile `\\x1b[>1u` → WT → 响应 `\\x1b[>1u` 或 `\\x1b[<flags u`
      → 目标 os.read

预期:
  - 收到含 `1B 5B 3E 31` 或 `1B 5B 3E` 前缀的应答 → PASS
        ...
        with TestSession() as s:
            ...
            if not v:
                ...
                if not v_got:
                    ...
                    print("  [UNSUPPORTED] 4s 无 Kitty 键盘协议应答（WT 不支持 CSI > 1 u）")
                else:
                    hexs = v_got.lower()
                    print("  [INFO] 响应 hex: {}".format(hexs))
                    if hexs.startswith("1b5b3e") and hexs.endswith("75"):
                        print("  [PASS] 收到 Kitty 键盘协议应答 1B 5B 3E ... 75(u)")
                    else:
                        print("  [FAIL] Kitty 应答格式异常: {}".format(hexs))
                        failures += 1
