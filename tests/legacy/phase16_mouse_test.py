...
在被注入的 cmd 中运行，用 ReadConsoleInputW 读取鼠标事件并记录到结果文件。
验证鼠标 VT 翻译链路中的坐标转换正确性：
...
功能：
  ...
  3. 鼠标事件：记录坐标/按钮状态/标志位到结果文件
  4. 键盘 'q'：退出

结果文件路径由环境变量 MOUSE_PHASE16_RESULT_FILE 指定，默认 ./mouse_phase16_result.txt
每行格式：
  MOUSE <x> <y> <buttonState> <eventFlags> <ctrlKeyState>
  KEY <vk> <ch> <bKeyDown>
  QUIT
...
def main():
    result_file = os.environ.get("MOUSE_PHASE16_RESULT_FILE", "mouse_phase16_result.txt")
    ...
    with open(result_file, "w", encoding="utf-8") as f:
        f.write("# mouse phase16 test result\n")

    # 屏幕提示
    msg = "Phase16 mouse test ready. Click anywhere, scroll wheel. Press 'q' to quit.\r\n"
