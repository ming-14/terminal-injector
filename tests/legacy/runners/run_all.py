...
SUITES = [
    ...
    ("phase10_pty", "test_phase10_pty", "Phase 10 屏幕渲染验证（WT + mediator 日志路径，替代 PTY-Agent）"),
    ...
    ("phase15", "test_phase15", "Phase 15 DSR/DA 终端属性查询验证（DSR CPR + DA 查询/响应）"),
    ("phase16", "test_phase16", "Phase 16 鼠标坐标状态管理（静态变量消除 + 坐标验证）"),
    ("phase17", "test_phase17", "Phase 17 字符宽度审计（CJK 双宽字符/Emoji 代理对宽度验证）"),
    ("phase18", "test_phase18", "Phase 18 滚动缓冲区一致性（Scrollback 计数/用户高度保留/模式切换重置）"),
]
