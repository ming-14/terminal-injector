...
#include <stdbool.h>
#include <stdlib.h>
#include <stdint.h>   // uint32_t (wcwidth32 参数类型)
...
int wcwidth32(uint32_t cp) {
	...
	// category code are of non-zero width.
        ...
        if (cp < 32 || (0x07F <= cp && cp < 0x0A0)) return -1;

        // Combining characters with zero width.
        if (intable(ZERO_WIDTH, sizeof(ZERO_WIDTH)/sizeof(struct width_interval), cp)) return 0;

        return intable(WIDE_EASTASIAN, sizeof(WIDE_EASTASIAN)/sizeof(struct width_interval), cp) ? 2 : 1;
}

int wcwidth(wchar_t ucs) {
        return wcwidth32((uint32_t)ucs);
}
