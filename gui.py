...
class InjectorGui(tk.Tk):
    ...
    def _build_toolbar(self):
        ...
        self.only_injectable_var = tk.BooleanVar(value=False)
    ...
    def _build_table(self):
        cols = ("pid", "name", "status", "arch", "console", "injected", "reason")
        wrap = ttk.Frame(self, padding=(6, 0))
        wrap.pack(fill="both", expand=True)
        self.tree = ttk.Treeview(wrap, columns=cols, show="headings",
                                 selectmode="browse")
        heads = {"pid": ("PID", 70, "center"),
                 "name": ("进程名", 170, "w"),
                 "status": ("状态", 90, "center"),
                 "arch": ("架构", 60, "center"),
                 ...
                 "reason": ("说明", 320, "w")}
        for cid, (text, width, anchor) in heads.items():
    ...
    def _build_log(self):
        ...
        wrap.pack(fill="both", expand=True, padx=6, pady=(4, 2))
        self.log_text = tk.Text(wrap, height=10, state="disabled",
    ...
    def refresh(self):
        """刷新进程列表(后台执行 --list-targets --json)"""
        self._run_task(self._fetch_targets, self._on_targets)
    ...
    def _render_table(self):
        ...
        for t in sorted(rows, key=lambda x: x["pid"]):
